    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-6">
        ...
        {!loading && !error && tab === 'categories' && (
          <CategoriesTab categories={categories} onReload={() => getCategories().then(setCategories)} />
        )}
        {!loading && !error && tab === 'rules' && (
          <RulesTab rules={rules} categories={categories} accounts={accounts} onReload={() => getCategoryRules().then(setRules)} />
        )}
      </main>
    </div>
  )
  ...
  return (
    <div className="grid grid-cols-2 gap-6">
      <div className="space-y-2">
        ...
        {showAdd === 'parent' && (
          <AddCategoryForm
            onSave={async ({ name, color }) => { await createCategory({ name, color }); setShowAdd(null); onReload() }}
            onCancel={() => setShowAdd(null)}
          />
        )}
        {categories.map(cat => (
          <CategoryRow key={cat.id} category={cat} selected={selectedParent?.id === cat.id} onClick={() => setSelectedParent(cat)} onReload={onReload} />
        ))}
      </div>
      ...
      <div className="space-y-2">
        {selectedParent ? (
          <>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wide">Sub-categories of {selectedParent.name}</h3>
              <button onClick={() => setShowAdd('child')} className="text-xs text-blue-400 hover:text-blue-300">+ Add</button>
            </div>
            {showAdd === 'child' && (
              <AddCategoryForm
                onSave={async ({ name, color }) => { await createCategory({ name, parent_id: selectedParent.id, color }); setShowAdd(null); onReload() }}
                onCancel={() => setShowAdd(null)}
              />
            )}
...
function CategoryRow({ category, selected, onClick, onReload }: {
  category: Category; selected?: boolean; onClick?: () => void; onReload: () => void
}) {
  ...
  async function handleSave() { await updateCategory(category.id, { name, color }); setEditing(false); onReload() }
  async function handleDelete() {
    if (!confirm(`Delete "${category.name}"? This cannot be undone.`)) return
    await deleteCategory(category.id); onReload()
  }
...
function AddCategoryForm({ onSave, onCancel }: { onSave: (v: { name: string; color: string }) => Promise<void>; onCancel: () => void }) {
...
function RulesTab({ rules, categories, accounts, onReload }: {
  rules: CategoryRule[]; categories: Category[]; accounts: Account[]; onReload: () => void
}) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-400">Rules match transaction descriptions and auto-assign a category.</p>
        <button onClick={() => setShowAdd(true)} className="text-sm bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg transition-colors">+ Add Rule</button>
      </div>

      {showAdd && (
        <AddRuleForm categories={flatCats} accounts={accounts}
          onSave={async p => { await createCategoryRule(p); setShowAdd(false); onReload() }}
                ...
                return (
                  <tr key={rule.id} className="border-b border-gray-800/50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => deleteCategoryRule(rule.id).then(onReload)} className="text-gray-500 hover:text-red-400 text-xs transition-colors">Delete</button>
                    </td>
                  </tr>
                )
              })}
...
function AddRuleForm({ categories, accounts, onSave, onCancel }: {
  ...
  async function handleSubmit() {
    ...
    setSaving(true)
    try { await onSave({ pattern: pattern.trim(), category_id: categoryId, account_id: accountId || undefined, priority }) }
...
function flattenCategories(tree: Category[]): Category[] {
  ...
  for (const cat of tree) { result.push(cat); if (cat.children) result.push(...cat.children) }
  return result
}
